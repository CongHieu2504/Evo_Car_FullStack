...
class M_shops_rows extends CI_Model {
    ...
    private function generate_where($args) {
        ...
        if (isset($args['cat_id_in'])) {
            $this->db->where_in($this->_table_name . ".listcatid", $args['cat_id_in']);
        }
        ...
        if (isset($args['brand'])) {
            $this->db->where($this->_table_name . ".brand", $args['brand']);
        }
        // Filter theo loại
        if (isset($args['type'])) {
    ...
    private function generate_order_by($args) {
        ...
        // Nếu không có order_by nào được set, mặc định lấy hàng mới nhất trước
        if (!isset($args['order_by']) || empty($args['order_by'])) {
            $this->db->order_by($this->_table_name . '.created', 'DESC');
