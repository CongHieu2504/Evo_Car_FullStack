...
class M_users extends CI_Model {
    ...
    private function generate_where($args) {
        ...
        if (isset($args['shop_q'])) {
            $this->db->group_start();
            $this->db->like($this->_table_name . ".full_name", $args['shop_q']);
            $this->db->or_like($this->_table_name . ".username", $args['shop_q']);
            $this->db->or_like($this->_table_name . ".shop_name", $args['shop_q']);
            $this->db->or_like($this->_table_name . ".shop_phone", $args['shop_q']);
            $this->db->or_like($this->_table_name . ".shop_address", $args['shop_q']);
            $this->db->group_end();
        }
        ...
        if (isset($args['shop_id'])) {
            $this->db->where($this->_table_name . '.shop_id', $args['shop_id']);
        }
        if (isset($args['not_in_shop_status'])) {
            $this->db->where_not_in($this->_table_name . ".shop_status", $args['not_in_shop_status']);
        }
        if (isset($args['shop_status'])) {
            $this->db->where($this->_table_name . '.shop_status', $args['shop_status']);
        }
    ...
    public function get($id) {
        $this->db->select($this->_table_name . '.*,' . 'groups_users.group_id, users_parent.full_name as parent_full_name, users_parent.username as parent_username, users_parent.refer_key as parent_refer_key, banker.name as banker_name');
        $this->db->from($this->_table_name);
        $this->db->join('groups_users', 'groups_users.userid = ' . $this->_table_name . '.userid', 'left');
        $this->db->join('groups', 'groups_users.group_id = ' . 'groups.group_id', 'left');
        $this->db->join('users as users_parent', 'users_parent.userid = ' .  $this->_table_name . '.referred_by', 'left');
    ...
    function validate_login($username, $password, $is_admin = FALSE) {
        $this->db->select($this->_table_name . '.*');  // Chỉ select từ bảng users
        $this->db->where('username', $username);
        $this->db->where('password', $password);
        $this->db->where('active', 1);
        if ($is_admin) {
            $this->db->where('role', 'ADMIN');  // Dùng role thay vì group_id
        }
        $this->db->from($this->_table_name);
        // Không join bảng groups_users vì không tồn tại
    ...
    function check_shop_id_availablity($shop_id) {
        if (trim($shop_id) == '') {
            return FALSE;
        }
